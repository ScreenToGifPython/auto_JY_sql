...
def call_llm(prompt: str, api_key: str, base_url: str, sql_type: str, model: str = LLM_MODEL, temperature: float = 0.1) -> str:
    sys_prompt = f"""
...
5. **执行效率：** 你写的 SQL 一定是执行效率最高的SQL代码, 绝对符合{sql_type}数据库的特性,语法,执行效率。
    ...
    if "lightcode-ui" in base_url:
        ...
        payload = {"model": "gpt-4o", "messages": [{"role": "system", "content": sys_prompt}, {"role": "user", "content": prompt}], "stream": False}
        ...
        resp = client.chat.completions.create(model=model, temperature=temperature, messages=[{"role": "system", "content": sys_prompt}, {"role": "user", "content": prompt}])
...
if __name__ == "__main__":
    ...
    parser.add_argument("--mode", type=str, default="sql", choices=["sql", "search"], help="执行模式: 'sql' (生成SQL) 或 'search' (表检索)")
    ...
    if args.mode == 'sql':
        for message in rag_sql_generator(question=args.question, api_key=args.key, base_url=args.url, sql_type=args.sql_type, embed_model_path=args.embed_model_path, top_k=args.k):
            print(message)
    elif args.mode == 'search':
        search_results = find_similar_tables(question=args.question, embed_model_path=args.embed_model_path, top_k=args.k)
